...
class calendarFrame:
    def __init__(
        ...
        s.G_Frame.pack()
        s.master.overrideredirect(
            1
        )
        s.master.update_idletasks()
        width, height = (
            s.master.winfo_reqwidth(),
            s.master.winfo_reqheight(),
        )
        if point and position:
            ...
            x, y = (
                s.master.winfo_screenwidth()
                - width
            ) / 2, (
                s.master.winfo_screenheight()
                - height
            ) / 2
        s.master.geometry(
            "%dx%d+%d+%d"
    ...
    def __get_calendar(
        ...
        if locale is None:
            ...
            return calendar.LocaleTextCalendar(
                fwday, locale
            )
    ...
    def __setitem__(
        ...
        if item in (
            ...
            s._canvas.itemconfigure(
                s._canvas.text,
                item=value,
            )
        else:
            s.G_Frame.__setitem__(
                s, item, value
            )
    ...
    def _build_calendar(s):
        ...
        header = (
            s._cal.formatmonthname(
                year, month, 0
            )
        )

        # 更新日历显示的日期
        cal = s._cal.monthdayscalendar(
            year, month
        )
        for (
            indx,
            item,
        ) in enumerate(s._items):
            week = (
                cal[indx]
                if indx < len(cal)
                else []
    ...
    def _pressed(
        ...
        if not item:
            ...
            item = widget.identify_row(
                y
            )
            column = widget.identify_column(
                x
            )

        if (
            not column
            or not item in s._items
        ):
            # 在工作日行中单击或仅在列外单击。
            return

        item_values = widget.item(
            item
        )["values"]
        if not len(
            item_values
